    ...
    $.extend({
        ...
        table: {
            ...
            init: function(options) {
                ...
                $('#' + options.id).bootstrapTable({
                    ...
                    onRefresh: options.onRefresh,                       // 刷新执行事件
            ...
            initEvent: function() {
                ...
                // 图片预览事件
                $(optionsIds).off("click").on("click", '.img-circle', function(event) {
            ...
            tooltip: function (value, length, target) {
                ...
                if (_value.length > _length) {
                    ...
                    _value = _value.replace(/\"/g,"&quot;");
                    var actions = [];
                    actions.push($.common.sprintf('<input style="opacity: 0;position: absolute;width:0px;z-index:-1" type="text" value="%s"/>', _value));
                ...
                $.modal.confirm("确定导出所有" + table.options.modalName + "吗？", function() {
                    ...
                    $.post(table.options.exportUrl, dataParam, function(result) {
                        if (result.code == web_status.SUCCESS) {
                            window.location.href = "/open/files?fileName=" + encodeURI(result.msg) + "&delete=" + true;
            ...
            selectFirstColumns: function(tableId) {
                var currentTableId = ($.common.isEmpty(tableId)?table.options.id:tableId);
                var rows = $.map($("#" + currentTableId).bootstrapTable('getSelections'), function (row) {
                    return $.common.getItemField(row, table.config[currentTableId].columns[1].field);
                });

                return $.common.uniqueFn(rows);
            },
            // 查询表格首列值
            selectSelections: function(tableId) {
                var currentTableId = ($.common.isEmpty(tableId)?table.options.id:tableId);
                return $("#" + currentTableId).bootstrapTable('getSelections');
            },
            // 回显数据字典
        ...
        operate: {
            ...
            submit: function(url, type, dataType, data, callback, contentType) {
                var config = {
                    ...
                    error: function(result){
                        $.modal.alertWarning(result.responseJSON.msg);
                        $.modal.closeLoading();
                    }
                };
                $.ajax(config)
            ...
            removeAll: function() {
                ...
                $.modal.confirm("确认要删除选中的" + rows.length + "条数据吗?", function() {
                    ...
                    $.operate.submit(url, "post", "json", JSON.stringify(rows), null, "application/json");
                });
            },
            // 清空信息
            clean: function() {
        ...
        tree: {
            ...
            searchNode: function(field) {
                ...
                // 根据搜索值模糊匹配
                $.tree.updateNodes($._tree.getNodesByParamFuzzy(field, value));
            },
            // 根据Id和Name选中指定节点
