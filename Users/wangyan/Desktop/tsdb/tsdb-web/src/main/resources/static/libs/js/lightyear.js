var lightyear = function(){
	 ...
	 * 页面loading
	 */
	var pageLoader = function($mode,$text) {
        if($text == undefined){
            $text = "";
        }
		...
		if ($mode === 'show') {
			if ($loadingEl.length) {
				...
				jQuery('body').prepend('<div id="lyear-loading"><div class="spinner-border text-primary" role="status"><span class="sr-only">Loading...</span></div><p class="spinner-text">'+$text+'</p></div>');
			}
		} else if ($mode === 'hide') {
			if ($loadingEl.length) {
